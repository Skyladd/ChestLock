...
class ChestLock extends PluginBase implements Listener {
	...
	public function onEnable()
	{
        ...
        $this->queue = [];
        $this->chestLockLogger = new ChestLockLogger($this->getDataFolder() . 'ChestLock.log');
        $this->databaseManager = new ChestLockDatabaseManager($this->getDataFolder() . 'ChestLock.sqlite3');
	...
	public function onCommand(CommandSender $sender, Command $command, $label, array $args)
	{
        ...
        switch (strtolower($command->getName())) {
            case "chest":
                ...
                $this->chestLockLogger->log("[" . $sender->getName() . "] Action:Command Command:" . $command->getName() . " Args:" . implode(",", $args));
                    ...
                    case "unlock":
                        ...
                        switch ($unlockOption) {
                            ...
                            case "all":
                                ...
                                $sender->sendMessage("Completed to unlock all chest");
                                break;
                            ...
                            case "player":
                                ...
                                if (is_null($target)) {
                                    ...
                                    $sender->sendMessage("/chestop unlock player <player>");
                                    return true;
                                }
                            ...
                            default:
                                $sender->sendMessage("/chestop unlock <all | player>");
                                return true;
                        }
                        break;
                      
                    default:
                        $sender->sendMessage("/chestop <unlock>");
                        return true;
                }
                $this->chestLockLogger->log("[" . $sender->getName() . "] Action:Command Command:" . $command->getName() . " Args:" . implode(",", $args));
                return true;
    ...
    public function onPlayerBreakBlock(BlockBreakEvent $event) {
        if ($event->getBlock()->getID() === Item::CHEST and $this->databaseManager->isLocked($event->getBlock())) {
            ...
            if ($owner === $event->getPlayer()->getName()) {
                ...
                $this->chestLockLogger->log("[" . $event->getPlayer()->getName() . "] Action:Unlock Level:{$chest->getLevel()->getName()} Coordinate:{$chest->x},{$chest->y},{$chest->z}");
    ...
    public function onPlayerInteract(PlayerInteractEvent $event) {
        ...
        if ($event->getBlock()->getID() === Item::CHEST) {
                ...
                switch ($taskName) {
                    case "lock":
                        if ($attribute === self::NOT_LOCKED) {
                            ...
                            $this->chestLockLogger->log("[" . $event->getPlayer()->getName() . "] Action:Lock Level:{$chest->getLevel()->getName()} Coordinate:{$chest->x},{$chest->y},{$chest->z}");
                        } else {
                    ...
                    case "public":
                        if ($attribute === self::NOT_LOCKED) {
                            ...
                            $this->chestLockLogger->log("[" . $event->getPlayer()->getName() . "] Action:Public Level:{$chest->getLevel()->getName()} Coordinate:{$chest->x},{$chest->y},{$chest->z}");
                        } else {
                            $event->getPlayer()->sendMessage("The chest has already been locked");
                        }
                        break;
                    ...
                    case "info":
                        if ($attribute !== self::NOT_LOCKED) {
                            ...
                            $this->chestLockLogger->log("[" . $event->getPlayer()->getName() . "] Action:Info Level:{$chest->getLevel()->getName()} Coordinate:{$chest->x},{$chest->y},{$chest->z}");
                        } else {
                    ...
                    case "passlock":
                        if ($attribute === self::NOT_LOCKED) {
                            ...
                            $this->chestLockLogger->log("[" . $event->getPlayer()->getName() . "] Action:Passlock Passcode:$passcode Level:{$chest->getLevel()->getName()} Coordinate:{$chest->x},{$chest->y},{$chest->z}");
                        } else {
                            $event->getPlayer()->sendMessage("The chest has already been locked");
                        }
                        break;
                    ...
                    case "passunlock":
                        if ($attribute === self::PASSCODE_LOCK) {
                            ...
                            if ($this->databaseManager->checkPasscode($chest, $passcode)) {
                                ...
                                if ($pairChestTile instanceof Chest) $this->databaseManager->unlock($pairChestTile);
                                $event->getPlayer()->sendMessage("Completed to unlock");
                                $this->chestLockLogger->log("[" . $event->getPlayer()->getName() . "] Action:Passunlock Level:{$chest->getLevel()->getName()} Coordinate:{$chest->x},{$chest->y},{$chest->z}");
                            } else {
                                $event->getPlayer()->sendMessage("Failed to unlock due to wrong passcode");
                                $this->chestLockLogger->log("[" . $event->getPlayer()->getName() . "] Action:FailPassunlock Level:{$chest->getLevel()->getName()} Coordinate:{$chest->x},{$chest->y},{$chest->z}");
                            }
                        } else {
            ...
            } elseif($attribute !== self::NOT_LOCKED and $attribute !== self::PUBLIC_LOCK and $owner !== $event->getPlayer()->getName() and !$event->getPlayer()->hasPermission("chestlock.op")) {
                $event->getPlayer()->sendMessage("The chest has been locked");
